...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 27
...
String wifiLastMessage = "Pluto9000 fallback ready";
...
String wifiStatusLabel() {
  ...
  if (wifiActive) return String(AP_NAME);
  return String("Web off");
}

String wifiIpLabel() {
  if (WiFi.status() == WL_CONNECTED) return WiFi.localIP().toString();
  if (wifiActive) return WiFi.softAPIP().toString();
  return String("0.0.0.0");
}

String wifiModeLabel() {
  if (!wifiActive) return String("Web off");
  if (WiFi.status() == WL_CONNECTED) return String("Router WiFi + Pluto fallback");
  return String("Pluto ad-hoc fallback");
}
...
void drawWifiScreen() {
  ...
  drawButton(14, 150, 292, 38, wifiActive ? "TURN WEBSERVER OFF" : "TURN WEBSERVER ON", wifiActive ? C_WARN : C_GOOD, wifiActive ? C_WARN : C_GOOD, TFT_BLACK, 2);
  drawButton(102, 202, 210, 34, "RESET WEB TO AP", C_BUTTON, C_GRID, C_TEXT, 2);
  drawBackButton();
}
...
String htmlPage() {
  ...
  h += F("<div class='card wifiCard'><div class='smallcap'>Milky connection guide</div><h2>WiFi / Web Control</h2><p class='note'>Pluto 9000 keeps an ad-hoc fallback named <b>Pluto9000</b>. Use it directly, or scan and join a router while the fallback stays available.</p><div class='statusGrid'><div><span class='smallcap'>SSID</span><b id='ssidNow'>"); h += wifiStatusLabel(); h += F("</b></div><div><span class='smallcap'>IP</span><b id='ipNow'>"); h += wifiIpLabel(); h += F("</b></div><div><span class='smallcap'>Web</span><b id='wifiState'>"); h += String(wifiActive ? "ON" : "OFF"); h += F("</b></div><div><span class='smallcap'>Port</span><b id='portNow'>"); h += String(webPort); h += F("</b></div></div><div class='label'>Share link</div><div class='share' id='wifiShare'>"); h += shareLink(); h += F("</div><div class='row'><button id='webToggleBtn' class='warn' onclick='turnWifiOff()'>TURN WEBSERVER OFF</button><button class='ghost' onclick='resetWebServer()'>RESET TO PLUTO9000</button></div><details open><summary>Connect to a wireless network</summary><p class='note'>Scan, pick your router, enter the password, and connect. Credentials are saved only after a successful connection.</p><button class='blue' onclick='scanWifi()'>SCAN WIFI</button><div class='label'>Networks found</div><select id='wifiList'><option>Press Scan WiFi</option></select><div class='label'>Password</div><input id='wifiPass' type='password' autocomplete='off' placeholder='router password'><button onclick='joinWifi()'>CONNECT AND SAVE</button></details><details><summary>Web port</summary><p class='note'>Default is 80. If you change this, the share link will include the port and the webserver should be reset.</p><input id='webPortInput' type='number' min='80' max='65535' value='"); h += String(webPort); h += F("'><button class='ghost' onclick='savePort()'>SAVE PORT</button></details><p class='mini' id='wifiMsg'>"); h += wifiLastMessage; h += F("</p></div>");
...
void handleState() {
  ...
  j += "\"ip\":\"" + wifiIpLabel() + "\",";
  j += "\"share\":\"" + shareLink() + "\",";
  j += "\"wifiMode\":\"" + wifiModeLabel() + "\",";
  j += "\"wifiMsg\":\"" + wifiLastMessage + "\",";
...
void handleWifiScan() {
  ...
  // Keep the existing Pluto fallback alive while scanning.
...
void handleWifiJoin() {
  ...
  wifiLastMessage = "Connecting to " + trySsid;
  logButtonPress("web wifi join");

  WiFi.mode(WIFI_AP_STA);
  // Keep the existing Pluto fallback alive while connecting.
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - start < 15000) {
    if (wifiActive && serverPtr != nullptr) server.handleClient();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    staSsid = trySsid;
    staPass = tryPass;
    wifiLastMessage = "Connected to " + WiFi.SSID() + " at " + WiFi.localIP().toString();
    markSettingsDirty();
    saveSettingsNow();
    logSdEvent("wifi sta connected");
    server.send(200, "text/plain", "CONNECTED: " + WiFi.localIP().toString());
  } else {
    wifiLastMessage = "Connect failed. Pluto9000 fallback still active.";
    logSystemProblem("wifi sta connect failed");
    server.send(200, "text/plain", "CONNECT_FAILED - Pluto9000 fallback still active");
  }
}
...
void handleWifiReset() {
  ...
  staSsid = "";
  staPass = "";
  wifiLastMessage = "Resetting to Pluto9000 fallback";
  markSettingsDirty();
  server.send(200, "text/plain", "RESETTING_TO_PLUTO9000");
...
void startWebServer() {
  ...
  bool apOk = WiFi.softAP(AP_NAME);
  if (!apOk) {
    wifiLastMessage = "AP start failed";
    logSystemProblem("wifi ap start failed");
  } else {
    wifiLastMessage = "Pluto9000 fallback ready";
  }

  // Try saved router credentials in the background while the AP remains available.
  if (staSsid.length() > 0) {
    WiFi.begin(staSsid.c_str(), staPass.c_str());
    wifiLastMessage = "Trying saved WiFi: " + staSsid;
  }
...
void loop() {
  ...
  if (wifiResetRequested) { stopWebServerNow(); startWebServer(); uiNeedsFullRedraw = true; }
