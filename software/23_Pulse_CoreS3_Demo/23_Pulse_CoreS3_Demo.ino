...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 23
...
bool nameLooksLikeLog(String n) {
  n.replace("/", "");
  if (n == "p9000.log") return true;
  if (!n.endsWith(".log")) return false;
  if (n.length() < 20) return false; // older timestamp format support
  return true;
}
...
void ensureStatsHtml() {
  ...
  f.println(F("</style></head><body><div class='wrap'><div class='hero'><div class='brand'>PulseCore / Pluto 9000 Stats</div><p class='sub'>This file is stored on the SD card as stats.html. It is a viewer only: the p9000.log file on the SD card is the source of truth. When opened from Pluto web control, this report reads those logs and turns it into simple charts and tables.</p></div>"));
  f.println(F("<div class='card'><div class='label'>Load log</div><select id='logSelect'></select> <button onclick='loadSelected()'>Load</button><p class='note'>If this file is opened directly from the SD card, browser security may block automatic log loading. Open it through Pluto web control at <b>192.168.4.1/stats.html</b>. The main software log is <b>p9000.log</b>, or use the file picker below.</p><input type='file' id='filePick' accept='.log,.csv'></div>"));
...
void beginSessionLog(bool resetFiles) {
  ...
  if (f) {
    if (newFile || resetFiles) {
      f.println("ms,uptime,event,vacTarget,vacActual,ppm,pulseWave,running,theme,backlightStep,heap,solPump,solPulse,solVac,solRelease");
    }
    f.close();
    sdDataReady = true;
  } else {
...
void bootAnimation() {
  ...
  // Entire boot animation is about 3 seconds.
  ...
  for (int i = 0; i < 18; i++) {
    int eyeDir = 0;
    if (i >= 3 && i < 7) eyeDir = 4;
    else if (i >= 7 && i < 11) eyeDir = -4;
    else if (i >= 11) eyeDir = 4;
    bootSprite.fillScreen(C_BG);
    drawMilkyToSprite(bootSprite, 160, 132, eyeDir);
    bootSprite.pushSprite(0, 0);
...
void drawMoreScreen() {
  ...
  M5.Display.setCursor(316 - tw, 230);
...
void drawWifiScreen() {
  ...
  M5.Display.setCursor(12, 69);
  M5.Display.print(wifiStatusLabel());
  M5.Display.setTextColor(C_TEXT, C_BG);
  M5.Display.setCursor(12, 102);
  M5.Display.print("IP:");
  M5.Display.setTextColor(C_ACCENT, C_BG);
  M5.Display.setCursor(52, 102);
  M5.Display.print(wifiIpLabel());
  M5.Display.setTextColor(C_TEXT, C_BG);
  M5.Display.setCursor(12, 132);
  M5.Display.print("Mode: ");
  M5.Display.print(wifiActive ? "Ad-hoc + web" : "Web off");
  drawButton(16, 160, 288, 40, wifiActive ? "TURN WEBSERVER OFF" : "TURN WEBSERVER ON", wifiActive ? C_WARN : C_GOOD, wifiActive ? C_WARN : C_GOOD, TFT_BLACK, 2);
  drawBackButton();
}
...
void drawDiagScreen() {
  ...
  drawButton(168, 48, 140, 52, pulseDemoMode ? "PULSE AUTO" : "PULSE", pulseDemoMode ? C_GOOD : C_BUTTON, pulseDemoMode ? C_GOOD : C_GRID, pulseDemoMode ? TFT_BLACK : C_TEXT, 2);
...
String htmlPage() {
  ...
  h += F("<section id='more' class='grid'><div class='card'><h2>More</h2><p class='note'>Extra tools live here so the dashboard stays clean. The stats report below is the easy way to read the data recorded on the SD card.</p><div class='row'><button onclick='api(\"/api/logsnap\")'>LOG SNAPSHOT</button><button class='blue' onclick='openStatsReport()'>OPEN STATS.HTML</button><button class='ghost' onclick='location.href=\"/p9000.log\"'>VIEW CURRENT LOG</button></div><p class='mini'>The stats page is served from the SD card file <b>/stats.html</b> and pulls data from the <b>p9000.log</b> on the same SD card.</p></div>");
...
h += F("<div class='card'><h2>SD Card Logging</h2><p><span class='pill'>SD: <span id='sdWeb'>"); h += sdReady ? "READY" : "NOT FOUND"; h += F("</span></span><span class='pill'>Logging: <span id='sdData'>"); h += sdDataReady ? "ON" : "NOT SET UP"; h += F("</span></span><span class='pill'>Events: <span id='sdEvents'>"); h += String(sdEventCount); h += F("</span></span></p><p class='note'><b>SD card is optional.</b> Pluto 9000 can run normally without an SD card. The SD card is only for recording logs, saving session stats, and creating <b>stats.html</b>.</p><details><summary>SD setup tools</summary><p class='note'>Use this only if you want data recording. This prepares Pluto data files on the SD card and clears/recreates Pluto-generated logs, <b>stats.html</b>, and <b>pluto9000.cfg</b>. It does not low-level format the card.</p><button class='ghost' onclick='showSdConfirm()'>SET UP / RESET SD LOGGING</button><div id='sdConfirm' class='confirmBox'><div class='dangerBox'><h2>Confirm SD setup</h2><p class='note'>This will reset Pluto-generated SD data files. The unit does not need an SD card to operate.</p><div class='row'><button class='danger' onclick='formatSd()'>YES, SET UP SD LOGGING</button><button class='ghost' onclick='hideSdConfirm()'>CANCEL</button></div></div></div><p class='mini' id='formatMsg'></p></details></div>");
...
void handleLogFile() {
  String path = sessionLogPath.length() ? sessionLogPath : String("/p9000.log");
  if (!sdReady || !SD.exists(path)) {
    server.send(404, "text/plain", "No p9000.log is set up yet. The SD card is optional; set up SD logging from More if you want logs recorded.");
    return;
  }
  File f = SD.open(path, FILE_READ);
  if (!f) { server.send(500, "text/plain", "Could not open p9000.log"); return; }
...
void handleWifiScan() {
  ...
  for (int i = 0; i < n; i++) {
    if (i) j += ",";
    j += "{\"ssid\":\"" + safeJsonString(WiFi.SSID(i)) + "\",\"rssi\":" + String(WiFi.RSSI(i)) + "}";
  }
  j += "]}";
  server.send(200, "application/json", j);
}
...
void startWebServer() {
  ...
  server.on("/p9000.log", handleLogFile);
...
void setup() {
  ...
  if (sdReady) {
    sdDataReady = SD.exists("/stats.html") || SD.exists("/p9000.log");
    if (sdDataReady) {
