...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 22
...
String wifiStatusLabel() {
  ...
  if (WiFi.isConnected()) return WiFi.SSID();
...
void drawThemeModeButton(int x, int y, int w, int h) {
  ...
  if (darkTheme) {
    M5.Display.fillCircle(x + 22, y + h / 2, 10, C_TEXT);
    M5.Display.fillCircle(x + 27, y + h / 2 - 2, 9, C_BUTTON);
    M5.Display.setTextSize(2);
    M5.Display.setTextColor(C_TEXT, C_BUTTON);
    M5.Display.setCursor(x + 42, y + 11);
    M5.Display.print("MOON");
  } else {
    M5.Display.fillCircle(x + 24, y + h / 2, 9, C_WARN);
    for (int i = 0; i < 8; i++) {
      float ang = i * 0.785398f;
      int x1 = x + 24 + (int)(cosf(ang) * 13);
      int y1 = y + h / 2 + (int)(sinf(ang) * 13);
      int x2 = x + 24 + (int)(cosf(ang) * 18);
      int y2 = y + h / 2 + (int)(sinf(ang) * 18);
...
void ensureStatsHtml() {
  ...
  f.println(F("</style></head><body><div class='wrap'><div class='hero'><div class='brand'>PulseCore / Pluto 9000 Stats</div><p class='sub'>This file is stored on the SD card as stats.html. It is a viewer only: the timestamped .log files on the SD card are the source of truth. When opened from Pluto web control, this report reads those logs and turns them into simple charts and tables.</p></div>"));
...
void drawMilkySideToSprite(M5Canvas& s, int cx, int cy, int spurtLen) {
  ...
  s.fillRoundRect(cx - 38, cy - 28, 76, 56, 14, TFT_WHITE);
  s.drawRoundRect(cx - 38, cy - 28, 76, 56, 14, M5.Display.color565(180, 205, 230));
  s.fillRoundRect(cx - 54, cy - 18, 20, 34, 7, M5.Display.color565(235, 245, 255));
  s.drawRoundRect(cx - 54, cy - 18, 20, 34, 7, M5.Display.color565(180, 205, 230));
  s.fillRect(cx - 58, cy - 14, 8, 26, M5.Display.color565(65, 155, 230));
  s.fillRoundRect(cx - 12, cy + 2, 42, 20, 7, blue);
  s.setTextSize(2);
  s.setTextColor(TFT_WHITE, blue);
  s.setCursor(cx - 8, cy + 6);
  s.print("milky");
  s.fillCircle(cx + 20, cy - 8, 7, TFT_BLACK);
  s.fillCircle(cx + 20, cy + 10, 7, TFT_BLACK);
  s.fillCircle(cx + 22, cy - 8, 3, TFT_WHITE);
  s.fillCircle(cx + 22, cy + 10, 3, TFT_WHITE);
  s.drawArc(cx + 6, cy + 18, 12, 8, 10, 150, TFT_BLACK);
  s.drawLine(cx - 12, cy + 28, cx - 18, cy + 52, TFT_BLACK);
  s.drawLine(cx + 10, cy + 28, cx + 16, cy + 52, TFT_BLACK);
  if (spurtLen > 0) {
    int sx = cx + 38;
    int sy = cy + 2;
    s.drawLine(sx, sy, sx + spurtLen, sy - 2, TFT_WHITE);
    s.drawLine(sx, sy + 2, sx + spurtLen - 8, sy + 5, TFT_WHITE);
    s.fillCircle(sx + spurtLen, sy - 2, 4, TFT_WHITE);
    s.fillCircle(sx + spurtLen - 10, sy + 6, 3, TFT_WHITE);
  }
}
...
void bootAnimation() {
  ...
  // Enter faster from the left.
  for (int x = -60; x <= 160; x += 18) {
    bootSprite.fillScreen(C_BG);
  ...
  // Quick look-around in the center.
  for (int i = 0; i < 20; i++) {
    int eyeDir = 0;
    if (i >= 4 && i < 8) eyeDir = 4;
  ...
  // Turn sideways and shoot milk to the right in spurts from halfway down the sprite.
  for (int i = 0; i < 10; i++) {
    int spurtLen = (i % 2 == 0) ? (26 + (i * 8)) : 0;
    bootSprite.fillScreen(C_BG);
    drawMilkySideToSprite(bootSprite, 145, 132, spurtLen);
    bootSprite.pushSprite(0, 0);
    vTaskDelay(55 / portTICK_PERIOD_MS);
  }

  // Exit to the right.
  for (int x = 160; x < 380; x += 20) {
    bootSprite.fillScreen(C_BG);
    drawMilkyToSprite(bootSprite, x, 132, 2);
    bootSprite.pushSprite(0, 0);
...
void drawDiagScreen() {
  ...
  drawButton(168, 48, 140, 52, solPulseOn ? "PULSE ON" : "PULSE", solPulseOn ? C_GOOD : C_BUTTON, solPulseOn ? C_GOOD : C_GRID, solPulseOn ? TFT_BLACK : C_TEXT, 2);
...
void applyHoldStep() {
  ...
  if (holdControl == 1) changeVac(held >= FAST_AFTER_MS ? -5 : -1);
  if (holdControl == 2) changeVac(held >= FAST_AFTER_MS ? 5 : 1);
  if (holdControl == 3) changePpm(held >= FAST_AFTER_MS ? -5 : -1);
  if (holdControl == 4) changePpm(held >= FAST_AFTER_MS ? 5 : 1);
...
void handleTouch() {
  ...
  if (t.wasPressed()) {
    ...
    if (currentScreen == SCREEN_DIAG) {
      ...
      if (insideRect(x, y, 168, 48, 140, 52)) { solPulseOn = !solPulseOn; applyHardwareOutputs(); uiNeedsFullRedraw = true; logSdEvent(solPulseOn ? "pulse output on" : "pulse output off"); return; }
...
String htmlPage() {
  ...
  h += F("<section id='more' class='grid'><div class='card'><h2>More</h2><p class='note'>Extra tools live here so the dashboard stays clean. The stats report below is the easy way to read the data recorded on the SD card.</p><div class='row'><button onclick='api(\"/api/logsnap\")'>LOG SNAPSHOT</button><button class='blue' onclick='openStatsReport()'>OPEN STATS.HTML</button><button class='ghost' onclick='location.href=\"/pluto9000.log\"'>VIEW CURRENT LOG</button></div><p class='mini'>The stats page is served from the SD card file <b>/stats.html</b> and pulls data from the timestamped <b>.log</b> files on the same SD card.</p></div>");
