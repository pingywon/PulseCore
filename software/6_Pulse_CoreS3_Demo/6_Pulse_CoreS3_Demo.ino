/*
  ...
  This revision:
  - Larger VAC and PPM adjustment controls
  - Press-and-hold repeat for + / - buttons
  - Smaller graphs so controls have more room
  - E-STOP moved smaller/tighter so it does not block screen controls
  - Carries forward dark/light mode, backlight slider, VAC/PPM 0 = OFF, and two-step E-STOP
*/
...
// Press-and-hold repeat handling for + / - controls.
int activeHoldControl = 0;
unsigned long holdStartMs = 0;
unsigned long lastHoldStepMs = 0;
static const unsigned long HOLD_FIRST_DELAY_MS = 350;
static const unsigned long HOLD_REPEAT_MS = 90;
static const int HOLD_NONE = 0;
static const int HOLD_PPM_MINUS = 1;
static const int HOLD_PPM_PLUS = 2;
static const int HOLD_VAC_MINUS = 3;
static const int HOLD_VAC_PLUS = 4;
static const int HOLD_LIGHT_MINUS = 5;
static const int HOLD_LIGHT_PLUS = 6;
...
void smallEstopButton() {
  // Small corner trigger only. It opens the full-screen confirmation and stays out of the working area.
  buttonRect(258, 8, 54, 24, "STOP", C_RED, C_WHITE, 1);
}
...
// ---------- Screen drawing forward declarations ----------
void drawPPMScreen();
void drawVACScreen();
...
void changeBacklightSetting(int delta) {
  backlightPercent += delta;
  if (backlightPercent < 0) backlightPercent = 0;
  if (backlightPercent > 100) backlightPercent = 100;
  applyBacklight();
}

int detectHoldControl(int x, int y) {
  if (currentScreen == SCREEN_PPM) {
    if (insideRect(x, y, 14, 164, 98, 66)) return HOLD_PPM_MINUS;
    if (insideRect(x, y, 118, 164, 98, 66)) return HOLD_PPM_PLUS;
  }
  else if (currentScreen == SCREEN_VAC) {
    if (insideRect(x, y, 14, 164, 98, 66)) return HOLD_VAC_MINUS;
    if (insideRect(x, y, 118, 164, 98, 66)) return HOLD_VAC_PLUS;
  }
  else if (currentScreen == SCREEN_SETTINGS) {
    if (insideRect(x, y, 18, 188, 76, 42)) return HOLD_LIGHT_MINUS;
    if (insideRect(x, y, 102, 188, 76, 42)) return HOLD_LIGHT_PLUS;
  }
  return HOLD_NONE;
}

void applyHoldStep(int control) {
  if (control == HOLD_PPM_MINUS) { changePPM(-2); drawPPMScreen(); }
  else if (control == HOLD_PPM_PLUS) { changePPM(2); drawPPMScreen(); }
  else if (control == HOLD_VAC_MINUS) { changeVAC(-2); drawVACScreen(); }
  else if (control == HOLD_VAC_PLUS) { changeVAC(2); drawVACScreen(); }
  else if (control == HOLD_LIGHT_MINUS) { changeBacklightSetting(-5); drawSettingsScreen(); }
  else if (control == HOLD_LIGHT_PLUS) { changeBacklightSetting(5); drawSettingsScreen(); }
}

void beginHoldIfControl(int x, int y) {
  activeHoldControl = detectHoldControl(x, y);
  if (activeHoldControl != HOLD_NONE) {
    holdStartMs = millis();
    lastHoldStepMs = holdStartMs;
  }
}

void updateHeldControl(bool stillPressed) {
  if (!stillPressed) {
    activeHoldControl = HOLD_NONE;
    return;
  }
  if (activeHoldControl == HOLD_NONE) return;

  unsigned long now = millis();
  if (now - holdStartMs >= HOLD_FIRST_DELAY_MS && now - lastHoldStepMs >= HOLD_REPEAT_MS) {
    lastHoldStepMs = now;
    applyHoldStep(activeHoldControl);
  }
}
...
void drawPPMScreen() {
  ...
  clearScreen();
  header("PPM CONTROL", true);

  M5.Display.setTextSize(4);
  M5.Display.setTextColor(C_PRIMARY);
  M5.Display.setCursor(24, 47);
  if (ppmValue <= 0) M5.Display.print("OFF");
  else M5.Display.print(ppmValue);

  M5.Display.setTextSize(2);
  M5.Display.setTextColor(C_TEXT);
  M5.Display.setCursor(146, 60);
  M5.Display.print("PPM");

  M5.Display.setTextSize(1);
  M5.Display.setTextColor(C_MUTED);
  M5.Display.setCursor(24, 87);
  M5.Display.print("Hold + / - to adjust squeezes per minute");

  drawGraphFrame(16, 101, 288, 54, "PPM rate visualizer");
  drawPPMWave(16, 101, 288, 54);

  buttonRect(14, 164, 98, 66, "-", C_PANEL2, C_TEXT, 5);
  buttonRect(118, 164, 98, 66, "+", C_PRIMARY_D, C_WHITE, 5);
  buttonRect(224, 164, 82, 66, "HOME", C_PANEL2, C_TEXT, 1);
}
...
void drawVACScreen() {
  ...
  M5.Display.setTextColor(C_MUTED);
  M5.Display.setCursor(24, 87);
  M5.Display.print("Hold + / - to adjust VAC from 0 to 100%");

  drawGraphFrame(16, 101, 288, 54, "VAC percent visualizer");
  drawVACWave(16, 101, 288, 54);

  buttonRect(14, 164, 98, 66, "-", C_PANEL2, C_TEXT, 5);
  buttonRect(118, 164, 98, 66, "+", C_ACCENT_D, C_WHITE, 5);
  buttonRect(224, 164, 82, 66, "HOME", C_PANEL2, C_TEXT, 1);
}
...
void handleTouch(int x, int y) {
  ...
  if (currentScreen == SCREEN_HOME) {
    ...
    if (insideRect(x, y, 14, 164, 98, 66)) {
      changePPM(-2);
      drawPPMScreen();
    } else if (insideRect(x, y, 118, 164, 98, 66)) {
      changePPM(2);
      drawPPMScreen();
    } else if (insideRect(x, y, 224, 164, 82, 66)) {
      activeHoldControl = HOLD_NONE;
      drawHome();
    }
  }
...
void setup() {
  ...
  Serial.println("6_Pulse_CoreS3_Demo loaded. UI simulation only.");
}
...
void loop() {
  ...
  if (touch.wasPressed()) {
    beginHoldIfControl(touch.x, touch.y);
    handleTouch(touch.x, touch.y);
  }

  updateHeldControl(touch.isPressed());
