...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 24
...
bool nameLooksLikeLog(String n) {
  ...
  // Daily format support: MM-DD-YYYY.log
  if (n.length() == 14 && n.charAt(2) == '-' && n.charAt(5) == '-') return true;
  // Older timestamp format support
...
void ensureStatsHtml() {
  ...
  f.println(F("</style></head><body><div class='wrap'><div class='hero'><div class='milky'><div class='neck'></div><div class='cap'></div><div class='bottle'></div><div class='face'>^^</div><div class='tag'>milky</div><div class='drop'></div><div class='puddle'></div></div><div><div class='brand'>Pluto 9000 Stats</div><p class='sub'>This report reads <b>p9000.log</b> and the daily log files on the SD card. The logs are the source of truth; this page turns the useful parts into readable stats.</p></div><div class='milky mascot2'><div class='neck'></div><div class='cap'></div><div class='bottle'></div><div class='face'>--</div><div class='tag'>milky</div><div class='drop'></div><div class='puddle'></div></div></div>"));
  f.println(F("<div class='card'><div class='label'>Load log</div><select id='logSelect'></select> <button onclick='loadSelected()'>Load</button><p class='note'>Open from Pluto web control at <b>192.168.4.1/stats.html</b>. If opened directly from the SD card, use the file picker.</p><input type='file' id='filePick' accept='.log,.csv'></div>"));
...
void beginSessionLog(bool resetFiles) {
  ...
  if (resetFiles && SD.exists("/p9000.log")) SD.remove("/p9000.log");
  ...
  bool newMaster = !SD.exists("/p9000.log");
  File m = SD.open("/p9000.log", FILE_WRITE);
  if (m) {
    if (newMaster || resetFiles) m.println(header);
    m.close();
  }
}
...
void appendCsvLine(const char* eventName) {
  ...
  if (sessionLogPath != "/p9000.log") appendCsvLineToPath("/p9000.log", eventName);
}
...
void setup() {
  ...
  if (sdReady) {
    sdDataReady = SD.exists("/stats.html") || SD.exists("/p9000.log") || SD.exists(dailyLogPath());
    if (sdDataReady) {
      beginSessionLog(false);
      logSdEvent("boot");
    }
