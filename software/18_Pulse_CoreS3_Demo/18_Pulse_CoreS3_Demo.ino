...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 18
...
void bootAnimation() {
  ...
  for (int i = 0; i < 110; i++) {
    int eyeDir = 0;
    if (i > 18 && i < 42) eyeDir = 4;
    else if (i >= 42 && i < 70) eyeDir = -4;
    else if (i >= 70 && i < 94) eyeDir = 4;
...
void handleLogFile() {
  if (!sdReady || !SD.exists("/pluto9000.log")) {
    server.send(404, "text/plain", "pluto9000.log is not set up yet. The SD card is optional; set up SD logging from More if you want logs recorded.");
    return;
  }
...
void handleSdFormat() {
  ...
  sdDataReady = true;
  ensureStatsHtml();
  File lf = SD.open("/pluto9000.log", FILE_WRITE);
  if (lf) { lf.println("Pluto 9000 log started"); lf.close(); }
...
void setup() {
  ...
  sdReady = SD.begin();
  if (sdReady) {
    sdDataReady = SD.exists("/stats.html") && SD.exists("/pluto9000.log");
    if (sdDataReady) logSdEvent("boot");
  }
