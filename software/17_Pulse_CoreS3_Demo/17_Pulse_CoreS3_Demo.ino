...
// =====================================================
//  PulseCore / Pluto 9000 - CoreS3/SE Demo Iteration 17
...
String htmlPage() {
  ...
  h += F("<section id='more' class='grid'><div class='card'><h2>More</h2><p class='note'>Extra tools live here so the dashboard stays clean. Use these when checking files, SD status, service data, or resetting the demo data on the SD card.</p><div class='row'><button onclick='api(\"/api/logsnap\")'>LOG SNAPSHOT</button><button class='ghost' onclick='location.href=\"/stats.html\"'>OPEN STATS</button><button class='ghost' onclick='location.href=\"/pluto9000.log\"'>VIEW LOG</button></div></div>");
  h += F("<div class='card'><h2>SD Card</h2><p><span class='pill'>SD: <span id='sdWeb'>"); h += sdReady ? "READY" : "NOT FOUND"; h += F("</span></span><span class='pill'>Events: <span id='sdEvents'>"); h += String(sdEventCount); h += F("</span></span></p><p class='note'>The card stores <b>pluto9000.log</b>, <b>stats.html</b>, and <b>pluto9000.cfg</b>.</p><div class='dangerBox'><h2>SD Formatter</h2><p class='note'>This demo formatter clears Pluto-generated files and recreates clean log/stats/config files. It does not low-level FAT-format the card.</p><input id='formatConfirm' type='text' placeholder='Type FORMAT to confirm'><button class='danger' onclick='formatSd()'>FORMAT SD DATA</button><p class='mini' id='formatMsg'></p></div></div>");
...
void handleSdFormat() {
  ...
  // This clears/recreates the Pluto 9000 generated files only.
  if (SD.exists("/pluto9000.log")) SD.remove("/pluto9000.log");
  ...
  lastSdLogMs = 0;
  ensureStatsHtml();
  logSdEvent("sd data formatted");
  saveSettingsNow();
  server.send(200, "text/plain", "SD_DATA_FORMATTED");
}
